    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="relative size-12 shrink-0 overflow-hidden rounded-full bg-ink-100">
          {seller.imageUrl && (
            <Image src={seller.imageUrl} alt={seller.name} fill className="object-cover" />
          )}
        </div>
        <div>
          <p className="font-semibold text-ink-900">{seller.name}</p>
          <p className="text-xs text-ink-500">{seller.title ?? "Asesor inmobiliario"}</p>
        </div>
      </div>
      ...
      <ContactForm propertyId={propertyId} agentId={seller.id} propertyTitle={propertyTitle} />
    </div>
  );
}
